# SOURCE: https://github.com/hima03/log-decorator
    ...
    def log_decorator_info(func):
        ...
        def log_decorator_wrapper(self, *args, **kwargs):
            ...
            args_passed_in_function = [
                repr(type(a)) for a in args
            ]  # my own addition: bcs of data protection and throughput
            # - I'd rather only check for the types of arguments passed
            ...
            kwargs_passed_in_function = [
                f"{k}={repr(type(v))}" for k, v in kwargs.items()
            ]
            ...
            logger_obj.info(
                f"Arguments: {formatted_arguments} - Begin function {func.__name__}",
                extra=extra_args,
            )
            try:
                ...
                logger_obj.info(
                    f"Returned: - End function {func.__name__} {type(value)!r}",
                    extra=extra_args,
                )
            except:
                """log exception if occurs in function"""
                logger_obj.error(
                    f"Exception: {str(sys.exc_info()[1])}", extra=extra_args
                )
                raise
